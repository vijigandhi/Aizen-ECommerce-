...
const MeatList = ({ searchTerm, storeId }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data && !response.data.error) {
          const spiceProducts = response.data.allProducts.filter(product => product.category_id === 11);
  ...
  return (
    <div>
      <h2 className="text-xl font-bold py-2">Meat, Fish & Eggs</h2>
