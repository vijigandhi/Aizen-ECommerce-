...
const SpicesList = ({ searchTerm, storeId }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data && !response.data.error) {
          const spiceProducts = response.data.allProducts.filter(product => product.category_id === 3);
  ...
  return (
    <div>
      <h2 className="text-xl font-bold py-2">Spices</h2>
