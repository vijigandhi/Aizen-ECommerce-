...
const OilsList = ({ searchTerm, storeId }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data && !response.data.error) {
          const vegetableProducts = response.data.allProducts.filter(product => product.category_id === 17);
  ...
  return (
    <div>
      <h2 className="text-xl font-bold py-2">Oils</h2>
