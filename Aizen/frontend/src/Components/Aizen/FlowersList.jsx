...
const FlowersList = ({ searchTerm, storeId }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data && !response.data.error) {
          const vegetableProducts = response.data.allProducts.filter(product => product.category_id === 16);
  ...
  return (
    <div>
      <h2 className="text-xl font-bold py-2">Fresh Flowers</h2>
