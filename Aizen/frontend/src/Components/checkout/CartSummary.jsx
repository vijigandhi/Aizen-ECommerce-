...
const CartSummary = ({ onTotalChange, onUserIdChange }) => {
  ...
  return (
    <div className="flex-1 bg-white shadow-md rounded-lg p-4 flex flex-col min-h-[38rem] max-h-[calc(100vh-200px)]">
      <h2 className="text-lg font-semibold mb-2">Order Summary</h2>
      {error && <p className="text-red-500">{error}</p>}
      <div className="flex-1 overflow-y-auto custom-scrollbar mb-4 p-2">
      {items.length > 0 ? (
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="flex justify-between text-lg font-semibold text-gray-900 border-t border-gray-200 pt-2">
          <span>Total:</span>
          <span>₹{(parseFloat(calculateSubtotal()) + parseFloat(calculateDelivery())).toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
